...
import {
	...
	replaceBlock,
	updateRecentWriting,
} from "./update-recent-writing.mjs";
...
test("a profile without writing markers disables feed fetching and updates", async () => {
	await updateRecentWriting({
		fetchImpl: async () => {
			throw new Error("Disabled writing must not fetch the feed");
		},
		readFileImpl: async () => "Profile with no recent-post section",
		writeFileImpl: async () => {
			throw new Error("Disabled writing must not write the profile");
		},
	});
});
...
test("updateRecentWriting aborts slow RSS fetches", async () => {
	...
	await assert.rejects(
		updateRecentWriting({
			fetchImpl,
			readFileImpl: async () =>
				"<!-- recent-writing:start --><!-- recent-writing:end -->",
			writeFileImpl: async () => {},
			timeoutMs: 1,
		}),
		/timed out after 1ms/,
